    ...
    <div>
      ...
      <div class="box">
        ...
        <q-scroll-area style="width: 200px; height: 100px;">
          <q-list v-for="inv in invites" :key="inv.email">{{ inv.email }}</q-list>
        </q-scroll-area>
      </div>
      <div class="box">
      ...
      <div class="box">
        <q-btn label="List Users" @click="listUsers"/>
        <br>
        <q-scroll-area style="width: 200px; height: 100px;">
          <q-list v-for="inv in users" :key="inv.email">{{ inv.email + " " + inv._id }}</q-list>
        </q-scroll-area>
      </div>
      <div class="box">
      ...
      <div class="box">
        ...
        <b>Is Private:</b>
        <q-input v-model="isPrivatePortal"/>
        <br>
        <b>Require Invite:</b>
        <q-input v-model="requiresInvite"/>
        <br>
        <b>Admin Email:</b>
        <q-input v-model="adminEmail"/>
        <br>
      ...
      <div class="box" style="background-color: red;">
        ...
        <b>Select New NodeType, Enter NodeId, Submit</b>
        <br>
...
export default {
  ...
  methods: {
    ...
    listInvites() {
      // alert('List invites')
      invites.find({ query: { $limit: 100 } }).then(response => {
        this.$data.invites = response.data;
      });
    },
    ...
    removeInvite() {
      ...
      json.userId = this.user._id;
      invites
        .find({ query: { email: ems } })
        .then(response => {
          // console.info('INVITE', JSON.stringify(response))
          invites
            .remove(response.data[0]._id)
            .then(response => {
              this.$q.notify({
                type: "positive",
                message: "Invitations removed"
              });
            })
            .catch(error => {
    ...
    listUsers() {
      users.find({ query: { $limit: 100 } }).then(response => {
    ...
    changeNodeType() {
      ...
      conversation
        .find({ query: { id: nid, skippop: true } })
        .then(response => {
...
*/
          ...
          conversation.update(child.id, child).then(resp => {
            if (parentId) {
              conversation
                .find({ query: { id: parentId, skippop: true } })
                .then(response2 => {
...
"questions":["72d22096-aef8-4e89-9f6d-ddaffd80dc36"]}*/
                  ...
                  conversation.update(parent.id, parent).then(resp2 => {
                    this.$data.nodeId = "";
                  });
                });
            }
