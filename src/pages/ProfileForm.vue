<template>
  <q-page :padding="true">
    <q-avatar>
      <q-item-side :avatar="thisUser.avatar"/>
    </q-avatar>
    <h6>Profile Edit Form</h6>
    <div>
      <b>Email</b>
      <br>
      <q-input v-model="myEmail" type="email"/>
      <br>
    </div>
    <div>
      <b>Full Name</b>
      <br>
      <q-input v-model="myName"/>
      <br>
    </div>

    <div>
      <b>Homepage URL</b> (Optional)
      <br>
    ...
    <div>
      <q-btn label="Change Password" @click="$router.replace('/PasswordChange')"/>
    </div>
  </q-page>
</template>
<script>
...
export default {
  ...
  methods: {
    doSubmit() {
      ...
      users.patch(usr._id, data).then(response => {
